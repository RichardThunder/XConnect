...
```bash
make build-darwin   # darwin/arm64 → dist/darwin-arm64/
...
| **macOS** | **`.pkg`**（官方安装器）、`*_darwin_arm64.zip`（仅 Apple Silicon） | 双击 pkg 安装：**应用程序** 中出现 **XConnect**（/Applications/XConnect.app），命令行工具在 `/usr/local/bin`；或解压 zip 使用 |
